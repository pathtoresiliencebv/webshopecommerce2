...
-- Get some coffee table products  
UPDATE products 
SET organization_id = 'aurelio-living-org-id'::uuid
...
-- First, we need to get the current user ID from auth.users
-- Since we can't directly access auth.users, we'll create the relationship
-- The admin will need to be logged in when this runs
