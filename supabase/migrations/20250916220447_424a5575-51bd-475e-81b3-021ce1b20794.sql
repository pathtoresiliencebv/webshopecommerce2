...
CREATE INDEX IF NOT EXISTS idx_products_active_org ON products(is_active, organization_id) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_product_attributes_product_type ON product_attributes(product_id, attribute_type);
...
CREATE INDEX IF NOT EXISTS idx_organization_users_user_org ON organization_users(user_id, organization_id) WHERE is_active = true;

-- Partial index for active categories by organization
CREATE INDEX IF NOT EXISTS idx_categories_active_org ON categories(organization_id, is_active) WHERE is_active = true;